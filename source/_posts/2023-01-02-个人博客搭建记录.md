...
之前都是在知乎上写博客，这样看的人也多，方便讨论，但是知乎的 markdown 编辑器不是很好用，本地写完后还需要一张一张上传图片，太麻烦了，就自己搭一个博客，又不想进行域名备案啥的，服务器也要出钱:(。最后搜到 Github 有提供免费的静态页面部署功能，这样写完markdown笔记可以直接上传到 Github，还可以通过网页访问，nice！(白嫖的感觉真好)
...
通过Github Pages，我们可以将Markdown撰写的笔记转换成静态网页，并进行部署。

> 官网：[https://pages.github.com/](https://pages.github.com/)

其创建流程也十分简单，只需要创建一个名为
...
<img src="/images/image-20230102153212301.png" alt="Hexo 官网" style="zoom:50%;" />

# Hexo 环境配置

## 安装

1. 全局安装 hexo-cli，这样我们直接调用hexo相关的命令行命令
...
5. 输入 `hexo s` 进行搭建本地服务

   ```bash
   (base) PS D:\Code\Note> hexo s
   INFO  Validating config
   INFO  Start processing
   INFO  Hexo is running at http://localhost:4000/ . Press Ctrl+C to stop.
   ```

   在浏览器中打开`localhost:4000`，就可以预览生成后的html效果了

   <img src="/images/image-20230102161237387.png" alt="image-20230102161237387" style="zoom: 33%;" />

## 主题

hexo上有很多精美的主题，这里我们选择 [next](https://github.com/theme-next/hexo-theme-next) 作为博客主题，其比较稳定，功能也很全。
...
hexo 提供了很多插件来简化我们的操作，并提供一些其他的功能。由于 hexo 是基于 node.js 实现的，
...
我们使用了以下几个插件（具体配置可以直接参考 next-theme 的插件使用文档：[Third-party Plugins | NexT](https://theme-next.js.org/docs/third-party-services/)）
...
这里我们主要调整生成 hexo 生成的markdown文件名称
...
`2023-01-02-Hello-World.md` 文件内容如下
...
前面提到，我们从`node_modules`中将next的配置拷贝至了根目录，并重命名为 `_config.next.yml`，主题相关的配置直接在这个文件中进行修改即可，next中提供了非常多的配置项，就不一一介绍了，详细可以直接参考Next主题文档：[NexT - Theme for Hexo](https://theme-next.js.org/)

## 图片

由于markdown和html在使用上的区别（markdown本地，html网络），我们在markdown中插入图片
...
但是一个是本地路径，一个是网络URI，直接这么转换肯定会问题。

参考 Hexo 的[资源管理文档]([Asset Folders | Hexo](https://hexo.io/docs/asset-folders))，在hexo中，最简单的图片管理方式就是将图片文件放置在`source/images`文件夹下，这样在网页中我们可以直接通过`/images/<image-name>`进行访问。
...
> 注意images前面的 `/` ，必须包含这个斜杠，否则图片路径就变成了相对当前URL的路径，会导致图片访问出错。
>
> 加上`/` 就表示是相对网址的URL，也就是 `https://purewhitevk.github.io/images/image.jpg`
>
> 而如果不包含`/`，假设我们当前路径为`https://purewhitevk.github.io/posts/abcd`，那么对应的图片URL为 `https://purewhitevk.github.io/posts/abcd/images/image.jpg`，这样就会导致找不到图片。
...
但是这样我们在本地使用 typora 编写 markdown 时就会有问题，好在 typora 也给出了解决方案：[Images in Typora - Typora Support](https://support.typora.io/Images/#relative-path-to-certain-folder)。

只需要在 markdown 的 Front Matter 中添加两个属性，将 markdown 中所有的图片路径都修改成相对某个路径的绝对路径即可（也就是`/relative/to/some-image.png`的形式）。
...
我们的文章都位于 `source/_posts/` 路径下，图片存储路径为`source/images`下，通过设置`typora-root-url`，当我们在markdown中插入一张路径为 `/images/image.jpg` 时，其会自动将 `typora-root-url`和该路径进行拼接，最终结果就是 `../images/image.jpg`，正确读取到图片。

另外一个便捷的功能就是插入图片时自动拷贝到指定文件夹，这样我们可以丝滑的管理图片了，而不需要手动上传并设置链接。

我们可以将这个部分添加到markdown模板（hexo中称为scaffold，脚手架）中，从而不用每次都进行添加，修改`scaffolds/post.md`
...
这样我们通过 `hexo new post <title>` 时就会默认配置好 typora，可以正常显示图片并在插入图片是进行自动拷贝了。

## 公式

另外一个比较令人头疼的就是公式渲染问题，latex 公式在语法上和 markdown 存在一些冲突，如果使用默认的 `hexo-renderer-marked `进行生成会导致最终渲染出来的 latex 公式是错误的，我们需要将markdown渲染引擎替换为 `hexo-renderer-pandoc`。
...
并在 next 主题的配置文件 `_config.next.yml` 中开启 MathJax的支持
...
最终进入 [purewhitevk.github.io](purewhitevk.github.io) 就可以访问到带有主题的结果了
